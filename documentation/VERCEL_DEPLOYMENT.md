...
- Le dépôt racine contient `vercel.json` configuré pour :
  ...
  - Utiliser `@vercel/python` sur `backend/manage.py` avec inclusion des dossiers `backend/**` et `frontend/**`.
  - Router `/static/*` vers `frontend/staticfiles` et toutes les autres routes vers Django.
